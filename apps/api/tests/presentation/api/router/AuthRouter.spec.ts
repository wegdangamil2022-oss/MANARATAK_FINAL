  ...
  describe('GET /api/v1/auth/csrf-token', () => {
    it('returns a token generated by the configured security service', async () => {
      const response = await supertest(app)
        ...
        .set('x-session-secret', 'test-session-secret');

      expect(response.status).toBe(200);
      expect(response.body.data.csrfToken).toBe('signed-csrf-token');
      expect(mockSecurityService.generateCsrfToken).toHaveBeenCalledWith('test-session-secret');
    });

    it('fails closed when no session secret is available', async () => {
      ...
      try {
        ...
        expect(response.body.error.code).toBe('CSRF_SECRET_UNAVAILABLE');
