...
describe('University translation workspace routes', () => {
  ...
  it('lists translations for the same canonical university id', async () => {
    ...
    const response = await request(createApp(useCases)).get('/admin/universities/university-1/translations');
  ...
  it('upserts an allowed locale without accepting a replacement canonical id', async () => {
    const useCases = createUseCases();
    useCases.upsertTranslation.mockResolvedValue({ locale: 'en', displayName: 'Test University', reviewStatus: 'APPROVED' });
    ...
    expect(payload).toMatchObject({ locale: 'en', displayName: 'Test University', reviewStatus: 'APPROVED' });
