...
export const FeaturedJobs: React.FC<FeaturedJobsProps> = ({ onViewAllClick }) => {
  return (
    <section className="px-0.5 sm:px-1 py-3 w-full font-['Cairo',sans-serif]">
      ...
      <div className="relative rounded-3xl p-4 sm:p-5 bg-gradient-to-b from-white to-slate-50/80 border border-slate-200/90 shadow-sm overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6 border-t-2 border-t-[#D9A93A]/40 min-h-[200px]">
        {/* Background Decorative Elements */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#D9A93A]/5 rounded-full blur-[60px] -translate-y-1/2 translate-x-1/4 pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-[#064D83]/5 rounded-full blur-[50px] translate-y-1/3 -translate-x-1/4 pointer-events-none"></div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-6 w-full">
          ...
          <div className="w-full md:w-7/12 text-center md:text-start flex flex-col items-center md:items-start">
            <div className="mb-3">
              <h3 className="text-sm sm:text-base font-bold text-slate-900 inline-flex items-center gap-1.5">
                <Briefcase className="w-4 h-4 text-[#064D83]" />
          ...
          <div className="hidden md:flex w-5/12 justify-center items-center relative">
            <div className="relative w-56 h-40">
              {/* Main Briefcase Card */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-[#064D83] rounded-2xl shadow-[0_8px_30px_rgba(15,75,58,0.2)] flex items-center justify-center z-20">
                <Briefcase className="w-8 h-8 text-white" />
              </div>

              {/* Floating Elements (Job Types & Icons) */}
              <div className="absolute top-2 right-8 w-10 h-10 bg-white rounded-xl shadow-md border border-slate-100 flex items-center justify-center z-10 transition-transform hover:-translate-y-1">
                <Building2 className="w-5 h-5 text-[#D9A93A]" />
              </div>

              <div className="absolute bottom-4 left-10 w-8 h-8 bg-white rounded-full shadow-md border border-slate-100 flex items-center justify-center z-10 transition-transform hover:-translate-y-1">
                <MapPin className="w-4 h-4 text-[#064D83]" />
              </div>

              <div className="absolute top-6 left-4 px-2.5 py-1 bg-white rounded-lg shadow-sm border border-slate-100 text-[9px] font-bold text-slate-600 -rotate-6 z-30">
                تدريب صيفي
              </div>

              <div className="absolute bottom-6 right-2 px-2.5 py-1 bg-white rounded-lg shadow-sm border border-slate-100 text-[9px] font-bold text-slate-600 rotate-6 z-30">
                عمل عن بعد
              </div>
