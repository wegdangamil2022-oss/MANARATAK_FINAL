...
export type TabType = 'home' | 'search' | 'favorites' | 'ai-tools' | 'tracker' | 'auth' | 'more';
      ...
      icon: (isActive: boolean) => (
        <Home
          className={`w-5 h-5 transition-transform duration-200 ${
            isActive
              ? 'stroke-[2.5] text-[#064D83] dark:text-[#E4B343] scale-110'
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {isActive && <span className="w-1.5 h-1.5 bg-[#D9A93A] rounded-full mt-0.5" />}
            </button>
          );
        })}
      </div>
