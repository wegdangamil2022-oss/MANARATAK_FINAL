...
const STUDENT_SERVICES = [
  {
    ...
    color: 'bg-blue-50/60 text-[#064D83]',
  },
  {
    id: 's2',
...
const CORPORATE_SERVICES = [
  ...
  },
  {
    ...
    color: 'bg-[#064D83]/10 text-[#064D83]',
  },
];
...
export const FeaturedServices: React.FC<FeaturedServicesProps> = ({ onViewAllClick }) => {
  ...
  return (
    <section className="px-0.5 sm:px-1 py-3 w-full font-['Cairo',sans-serif]">
      {/* Container */}
      <div className="relative rounded-3xl p-4 sm:p-5 bg-gradient-to-b from-white to-slate-50/80 border border-slate-200/90 shadow-sm overflow-hidden border-t-2 border-t-[#D9A93A]/40">
        ...
        <div className="flex flex-col items-center gap-4 mb-5">
          <div className="text-center">
            <h3 className="text-sm sm:text-base font-bold text-slate-900 inline-flex items-center justify-center gap-1.5">
              <Briefcase className="w-4 h-4 text-[#D9A93A]" />
          ...
          <div className="relative p-[2px] rounded-[14px] overflow-hidden group w-full max-w-[320px] shrink-0">
            ...
            <div className="relative flex bg-slate-50 p-1 rounded-xl w-full">
              ...
              >
                <GraduationCap
                  className={`w-3.5 h-3.5 ${activeTab === 'students' ? 'text-[#D9A93A]' : 'text-slate-400'}`}
                />
              ...
              >
                <Building2
                  className={`w-3.5 h-3.5 ${activeTab === 'corporate' ? 'text-[#D9A93A]' : 'text-slate-400'}`}
                />
        ...
        >
          {(activeTab === 'students' ? STUDENT_SERVICES : CORPORATE_SERVICES).map((service) => (
            <div
              key={service.id}
              className="group bg-white border border-slate-100 hover:border-[#D9A93A]/30 rounded-xl p-3 flex items-center gap-3 shadow-sm hover:shadow-md transition-all cursor-pointer relative overflow-hidden"
            >
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-bold text-xs sm:text-sm text-slate-800 group-hover:text-[#064D83] transition-colors truncate">
                  {service.title}
                </h4>
              ...
              {/* Action Icon */}
              <div className="shrink-0 w-6 h-6 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-[#064D83] transition-all">
