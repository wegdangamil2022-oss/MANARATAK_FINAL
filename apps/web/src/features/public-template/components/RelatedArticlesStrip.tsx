  ...
  return (
    <section className={`rounded-2xl border border-[var(--mn-border-brand)]/25 bg-[var(--mn-surface)] shadow-sm  mn-panel ${compact ? 'p-3' : 'p-3.5'}`}>
      <div className="mb-2 flex items-center gap-2">
        ...
        <div>
          <h3 className="text-[11px] font-black text-[var(--mn-heading)]">مقالات وأدلة مرتبطة</h3>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
        {articles.map((article) => (
          ...
          >
            <div className="flex items-center gap-1.5 text-[8px] font-black text-[var(--mn-accent-text)]">
              <span>{article.typeLabel || 'مقال'}</span>
              {article.category && <span className="text-[var(--mn-text-muted)]">• {article.category}</span>}
            </div>
            <p className="mt-1 text-[10px] font-black leading-5 text-[var(--mn-heading)] line-clamp-2">{article.title}</p>
