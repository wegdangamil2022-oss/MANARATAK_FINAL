...
check('P12-ADM-007', /countryReferenceId/.test(repo) && /countrySourceLabel/.test(repo) && !/where\.optionalFields\s*=\s*\{ path: \['studyCountry'\]/.test(repo), 'admin country filter uses normalized fields');
check('P12-PUB-001', /degreeLevel/.test(contracts) && /fundingCoverage/.test(contracts) && /sponsorName/.test(contracts) && /applicationDeadlineFrom/.test(contracts) && /where\.degreeTargets\s*=\s*\{\s*some:/.test(repo) && /where\.applicationDeadline\s*=\s*\{/.test(repo), 'public filters are represented and queried');
