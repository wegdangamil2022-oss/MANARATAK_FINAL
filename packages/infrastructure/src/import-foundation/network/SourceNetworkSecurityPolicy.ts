...
export class SourceNetworkSecurityPolicy {
  constructor(private readonly resolver: SourceAddressResolver = new NodeSourceAddressResolver(), private readonly addresses = new PublicNetworkAddressPolicy()) {}
  async validate(source: ImportSourceDefinition, value: string): Promise<ValidatedSourceTarget> {
    ...
    if (!addresses.length || addresses.some((address) => !this.addresses.isPublic(address))) throw new Error('SOURCE_ADDRESS_BLOCKED');
