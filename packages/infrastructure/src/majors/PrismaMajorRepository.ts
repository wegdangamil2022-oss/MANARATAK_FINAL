...
export class PrismaMajorRepository implements IMajorRepository {
  ...
  private matchesFilters(major: MajorDto, filters: MajorFilters): boolean {
    ...
    if (filters.degreeLevel) {
      const matchLevel = filters.degreeLevel.toLowerCase();
      // Try canonical relation first
      const hasCanonicalMatch = major.profiles?.some(profile => {
        const canonical = profile.degreeLevel;
        if (!canonical) return false;
        return (
          canonical.canonicalCode.toLowerCase() === matchLevel ||
          canonical.nameEn.toLowerCase() === matchLevel ||
          canonical.nameAr.toLowerCase() === matchLevel
        );
      });
      if (!hasCanonicalMatch) {
        // Fallback to legacy text comparison
        const legacyLevel = getText(major.degreeLevel ?? optionalFields.degreeLevel);
        if (legacyLevel !== matchLevel) {
          return false;
        }
      }
    }
    ...
    if (filters.academicFieldOrDiscipline) {
      const matchText = filters.academicFieldOrDiscipline.toLowerCase();
      // Try canonical academicField/discipline first
      const fieldNode = major.academicField;
      const disciplineNode = major.discipline;
      
      const hasCanonicalMatch = (
        (fieldNode && (
          fieldNode.canonicalCode.toLowerCase().includes(matchText) ||
          fieldNode.canonicalName.toLowerCase().includes(matchText)
        )) ||
        (disciplineNode && (
          disciplineNode.canonicalCode.toLowerCase().includes(matchText) ||
          disciplineNode.canonicalName.toLowerCase().includes(matchText)
        ))
      );
      
      if (!hasCanonicalMatch) {
        // Fallback to legacy text comparison
        const fieldText = getText(major.academicFieldOrDiscipline ?? optionalFields.academicFieldOrDiscipline);
        if (!fieldText.includes(matchText)) {
          return false;
        }
      }
    }
    ...
    if (filters.search) {
      ...
      // Build a robust canonical + legacy haystack
      const searchTerms = [
        major.displayName,
        major.canonicalName,
        major.slug,
        ...
        college,
        getText(major.degreeLevel ?? optionalFields.degreeLevel),
        getText(major.academicFieldOrDiscipline ?? optionalFields.academicFieldOrDiscipline),
      ];

      if (major.academicField) {
        searchTerms.push(major.academicField.canonicalCode);
        searchTerms.push(major.academicField.canonicalName);
      }
      if (major.discipline) {
        searchTerms.push(major.discipline.canonicalCode);
        searchTerms.push(major.discipline.canonicalName);
      }

      major.profiles?.forEach(profile => {
        searchTerms.push(profile.displayName);
        searchTerms.push(profile.localizedNameAr);
        searchTerms.push(profile.localizedNameEn);
        if (profile.degreeLevel) {
          searchTerms.push(profile.degreeLevel.canonicalCode);
          searchTerms.push(profile.degreeLevel.nameEn);
          searchTerms.push(profile.degreeLevel.nameAr);
        }
        if (profile.academicField) {
          searchTerms.push(profile.academicField.canonicalCode);
          searchTerms.push(profile.academicField.canonicalName);
        }
        if (profile.discipline) {
          searchTerms.push(profile.discipline.canonicalCode);
          searchTerms.push(profile.discipline.canonicalName);
        }
      });

      const haystack = searchTerms.map((value) => typeof value === 'string' ? value.toLowerCase() : '').join(' ');
