...
describe('JwtTokenProvider infrastructure adapter', () => {
  it('generates and verifies access and refresh tokens', async () => {
    ...
    await expect(provider.verifyRefreshToken(tokens.refreshToken)).resolves.toMatchObject({ userId: 'user-123', sessionId: 'session-123' });
  });

  it('rejects swapping refresh and access tokens without leaking verification details', async () => {
    ...
    await expect(provider.verifyRefreshToken(tokens.accessToken)).rejects.toThrow(/^Invalid refresh token$/);
  });
