import { IStudentToolHandler, IStudentToolDependencyHealthGateway } from './contracts';
...
export class StudentToolActivationReadinessService { constructor(private readonly handlers: StudentToolHandlerRegistry, private readonly health: IStudentToolDependencyHealthGateway) {} async evaluate(tool: StudentToolDefinition) { const blockers: string[] = []; if (tool.implementationStatus !== StudentToolImplementationStatus.IMPLEMENTED) blockers.push('TOOL_NOT_IMPLEMENTED'); if (!this.handlers.has(tool.toolKey)) blockers.push('MISSING_IMPLEMENTATION_HANDLER'); if (!tool.inputSchema.fields.length || !tool.outputSchema.fields.length) blockers.push('INVALID_TOOL_SCHEMA'); if (tool.availability.adminOnly && tool.availability.publicEnabled) blockers.push('CONFLICTING_AVAILABILITY_FLAGS'); if (!tool.rateLimitPolicy || Object.values(tool.rateLimitPolicy).some((value) => !Number.isInteger(value) || value < 1)) blockers.push('INVALID_RATE_LIMIT_POLICY'); if (!tool.featureFlags.globallyEnabled) blockers.push('TOOL_GLOBALLY_DISABLED'); for (const dependency of tool.dependencies.filter((item) => item.required)) { const status = await this.health.status(dependency); if (status !== 'READY') blockers.push(`${dependency.phase}_${status}`); } return { ready: blockers.length === 0, blockers }; } }
export class StudentToolHealthService { constructor(private readonly handlers: StudentToolHandlerRegistry, private readonly dependencies: IStudentToolDependencyHealthGateway) {} async compute(tool: StudentToolDefinition): Promise<StudentToolHealthStatus> { if (tool.featureFlags.maintenanceMode) return StudentToolHealthStatus.MAINTENANCE; if (tool.implementationStatus !== StudentToolImplementationStatus.IMPLEMENTED || !this.handlers.has(tool.toolKey)) return StudentToolHealthStatus.NOT_CONFIGURED; if (tool.lifecycle === StudentToolLifecycleStatus.RETIRED || tool.visibility === StudentToolVisibilityStatus.DISABLED) return StudentToolHealthStatus.OFFLINE; const states = await Promise.all(tool.dependencies.filter((item) => item.required).map((item) => this.dependencies.status(item))); if (states.some((state) => state === 'UNAVAILABLE')) return StudentToolHealthStatus.OFFLINE; if (states.some((state) => state !== 'READY')) return StudentToolHealthStatus.DEGRADED; return StudentToolHealthStatus.HEALTHY; } }
